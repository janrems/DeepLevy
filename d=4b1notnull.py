...
def _one_layer (input_ , output_size ,activation_fn =tf.nn.relu ,stddev =5.0 , name ='linear'): # kle mas activation function. tole je funkcija za en nivo
...
with tf . Session () as sess :
  ...
  with tf . variable_scope ( 'forward',reuse=tf.compat.v1.AUTO_REUSE ):
    ...
    for i in range (0, N_t2):
      ...
      for j in range (0, batch_size):
        XI[i,j,:].assign( b1*X[i,j,:]*dt + dr2 + sqrdt * sigma * dW[j,:] )
      dW = tf . random_normal ( shape =[batch_size,d] , stddev = 1 , dtype = tf . float64 ) 
    X=tf.concat([X0, XI], 0)
    integration1 = 0.5*tf.reduce_sum(tf.multiply(X[0,:,:],X[0,:,:]),1)     #???????????????
    integration2 = tfp.math.trapz(0.5*tf.reduce_sum(tf.multiply(X[0:N_delta,:,:],X[0:N_delta,:,:]),2), axis=0)
    integration3= tfp.math.trapz(tf.reduce_mean(tf.reduce_sum(tf.multiply(X[N_delta:N_time,:,:],X[N_delta:N_time,:,:]),
                                                                                     2),axis=1), axis=0 )                        
    #Cost function
  ...
  except KeyboardInterrupt : print ( "manually disengaged" )
print("\n,N_delta:",N_delta)    
print("\n,iii:",i)
print("\n,diff:",diff)
print("\n,X0:",Xl[0:N_delta,1,0])
